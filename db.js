      ...
      for(i=0; i<10; i++){
        ...
        async.parallel({
          ...
          chassis: node.getChassis,
             ...
             console.log('vendor: ', results.vendor.name, 'chassis: ', results.chassis.name);
           })
        }
    });
  });
          ...
          async.series([
            function(cbg){
              async.eachSeries(fixtures.fixturesChassis(), function(row, cb){
                insertChassis(db, row, function(){console.log('chassis inserted'); cb()});
              }, function(err){
            ...
            function(cbg){
              async.eachSeries(fixtures.fixturesNodes(), function(row, cb){
                insertNodes(db, row, function(){console.log('node inserted'); cb()});
              }, function(err){
